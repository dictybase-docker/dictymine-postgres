...
create_intermine_role_and_db() {
    if [ "${INTERMINE_USER+defined}" -a "${INTERMINE_PASSWORD+defined}" -a "${INTERMINE_DB+defined}" ]
    then
...
create_intermine_items_db() {
    if [ "${INTERMINE_USER+defined}" -a "${INTERMINE_PASSWORD+defined}" -a "${INTERMINE_ITEMS_DB+defined}" ]
    then
        psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" <<-EOSQL
...
create_intermine_userprofile_db() {
    if [ "${INTERMINE_USER+defined}" -a "${INTERMINE_PASSWORD+defined}" -a "${INTERMINE_USERS_DB+defined}" ]
    then
        psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" <<-EOSQL
